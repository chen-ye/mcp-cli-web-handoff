...
- [ ] Task: Reconfigure MCP Server Coverage
    - [ ] **Implement:** Update the root `package.json` `test:unit` and `coverage` scripts to output `c8` reports to `coverage/mcp-server/` instead of the root `coverage/` directory.
    - [ ] **Implement:** Update `.gitignore` if necessary to ensure the new folder structure is ignored.
